...
x_axis = np.loadtxt('AFMPowderAverage.x')
E_array = np.loadtxt('AFMPowderAverage.y')
image_array = np.loadtxt('AFMPowderAverage.mat')

print len(image_array)
print len(image_array[0])
print np.max(image_array)
im = ax.pcolorfast(x_axis[:,2]/(2.0*np.pi),E_array,image_array,vmin=0.0,vmax=1.0)
...
fig.colorbar(im);
plt.savefig('AFMavg.png',dpi=400,bbox_inches='tight')
plt.close()
