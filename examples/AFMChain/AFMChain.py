...
dispersion = np.transpose(np.loadtxt('AFMChain.txt'))
image_array = np.transpose(np.loadtxt('AFMcut.txt'))


print len(image_array)
print len(image_array[0])
print np.max(image_array)
im = ax.pcolorfast(x_axis,E_array,image_array,vmin=0.0,vmax=3.0)
