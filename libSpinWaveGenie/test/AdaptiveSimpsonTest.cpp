...
BOOST_AUTO_TEST_CASE(ConstantFunctionTest)
{
  std::function<std::vector<double>(std::deque<double> & x)> f =
      std::bind<std::vector<double>>(constantFunction, std::placeholders::_1);
...
BOOST_AUTO_TEST_CASE(SphericalHarmonicsTest)
{
  for (unsigned n1 = 0; n1 < 4; ++n1)
  {
    for (unsigned n2 = 0; n2 < 4; ++n2)
    {
      ...
      std::function<std::vector<double>(std::deque<double> & x)> f =
          std::bind<std::vector<double>>(&SphericalHarmonics::getPoint, &myClass, std::placeholders::_1);
      AdaptiveSimpson test;
      test.setFunction(f);
...
{
  std::function<std::vector<double>(std::deque<double> & x)> f =
      std::bind<std::vector<double>>(doubleGaussianFunction, std::placeholders::_1);
  AdaptiveSimpson test;
  test.setFunction(f);
  std::vector<double> lower_bounds = {0.0, 0.0};
  std::vector<double> upper_bounds = {1.0, 1.0};
...
{
  std::function<std::vector<double>(std::deque<double> & x)> f =
      std::bind<std::vector<double>>(tsudasFunction, std::placeholders::_1);
  AdaptiveSimpson test;
  test.setFunction(f);
  std::vector<double> lower_bounds = {0.0, 0.0};
  std::vector<double> upper_bounds = {1.0, 1.0};
