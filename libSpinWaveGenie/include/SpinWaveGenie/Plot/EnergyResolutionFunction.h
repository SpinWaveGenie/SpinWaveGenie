...
template<class T>
class EnergyResolution : public SpinWavePlot{
public:
    EnergyResolution(){};
...
template<class T>
EnergyResolution<T>::EnergyResolution(std::unique_ptr<OneDimensionalShapes> ResolutionFunctionIn, T SWIn, Energies energiesIn)
{
...
template<class T>
EnergyResolution<T>::EnergyResolution(const EnergyResolution<T>& other)
{
...
template<class T>
EnergyResolution<T>& EnergyResolution<T>::operator=(EnergyResolution& other)
{
    std::cout << "Copying Energy Resolution Function" << std::endl;
    energies = other.energies;
    SW = other.SW;
    ...
    template<class T>
    void EnergyResolution<T>::setSpinWave(T SWIn)
    {
        SW = SWIn;
    }
    
    template<class T>
    std::vector<double> EnergyResolution<T>::getCut(double kx, double ky, double kz)
    {
        //cout << "Energy Points: " << EnergyPoints << endl;
    ...
    template<class T>
    const Cell& EnergyResolution<T>::getCell() const
    {
        return SW.getCell();
    }
    
    template<class T>
    const Energies& EnergyResolution<T>::getEnergies()
    {
        return energies;
    }
    
    template<class T>
    void EnergyResolution<T>::setEnergies(Energies energiesIn)
    {
        energies = energiesIn;
    }
    
    template<class T>
    std::unique_ptr<SpinWavePlot> EnergyResolution<T>::clone()
    {
