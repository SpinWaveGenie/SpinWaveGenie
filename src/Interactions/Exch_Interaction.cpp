...
void Exch_Interaction::Update_Matrix(Vector3d K, boost::shared_ptr<Cell> cell, MatrixXcd &LN, int quadrant)
{
    ...
    /*switch (quadrant)
    {
        case 0:
            ...
            break;
        case 1:
            LN(r,s+M) -= 0.25*z_rs*X*(conj(gamma_rs)*conj(G2)+gamma_rs*conj(G2));
            break;
        case 2:
            LN(r+M,s) -= 0.25*z_rs*X*(conj(gamma_rs)*G2+gamma_rs*G2);
            break;
        case 3:
    ...
    switch (quadrant)
    {
        case 0:
            ...
            LN(r,s) += 0.25*z_rs*X*gamma_rs*G1;
            LN(s,r) += 0.25*z_rs*X*gamma_rs*conj(G1);
            LN(s,s) += 0.25*z_rs*value*Sr*Frs(2,2);
            break;
        case 1:
            LN(r,s+M) += 0.25*z_rs*X*conj(gamma_rs)*conj(G2);
            LN(s,r+M) += 0.25*z_rs*X*gamma_rs*conj(G2);
            break;
        case 2:
            LN(r+M,s) += 0.25*z_rs*X*conj(gamma_rs)*G2;
            LN(s+M,r) += 0.25*z_rs*X*gamma_rs*G2;
            break;
        case 3:
            LN(r+M,r+M) += 0.25*z_rs*value*Ss*Frs(2,2);
            LN(r+M,s+M) += 0.25*z_rs*X*conj(gamma_rs)*conj(G1);
            LN(s+M,r+M) += 0.25*z_rs*X*conj(gamma_rs)*G1;
